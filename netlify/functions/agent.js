...
exports.handler = async (event, context) => {
    ...
    const { id: videoId, format: requestedFormat } = event.queryStringParameters;
    if (!videoId || !requestedFormat) {
        return { statusCode: 400, body: JSON.stringify({ success: false, message: 'Error: Missing video ID or format.' }) };
    }
    ...
    try {
        ...
        return { statusCode: 500, body: JSON.stringify({ success: false, message: `Agent Error: ${error.message}` }) };
    }
};
